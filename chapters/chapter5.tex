...
除了上述标准任务轨迹外，本章还专门采集了一定比例的纠错轨迹。本章采用扩散模型作为动作生成模型，若训练数据仅包含理想的专家运动轨迹，模型在推理时一旦因累积误差或外部干扰偏离了专家轨迹，便往往难以自主调整并恢复到正确的执行路径。为此，操作者在数据采集过程中，会故意引导机械臂向各个错误方向运动（例如偏离抓取目标或误触障碍物），随后再手动将其纠正回正确的操作路径。这部分纠错轨迹被补充到训练集中，占总数据量的10\%至15\%，使模型具备了在偏离专家轨迹后主动恢复的能力。图~\ref{fig:correction_trajectories_placeholder}~展示了纠错轨迹的典型样例。
