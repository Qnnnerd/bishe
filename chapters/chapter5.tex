...
在完成原始数据的采集后，系统需要将其转换为模型训练所需的标准格式。本研究遵循机器人学习数据系统（RLDS，Robotics Learning Data System）的数据规范，开发了定制化的数据集构建脚本。在数据处理过程中，脚本首先读取每帧记录的真实关节位置，利用UR5e的正运动学模型与DH参数，计算出包含工具中心点偏移的末端执行器位姿。随后，通过计算相邻两帧末端位姿的三维平移差值与旋转向量差值，结合夹爪的开合状态，构建出7维的动作标签向量。这种基于正运动学重建与差分计算的方法，减少了直接使用原始控制指令可能引入的执行器偏差。

图~\ref{fig:action_distribution_placeholder}~展示了自建数据集与LIBERO数据集的动作分布对比。从图中分析可知，在真实机器人的遥操作采集中，为了保证设备与环境的安全性，操作者通常会采取更为保守的控制策略。这导致真实数据集中的每组动作序列包含更多的执行步数，且单步动作的位姿变化量小于仿真环境中的动作幅度。这也是本章在真实场景下进行专项微调的动因之一。
