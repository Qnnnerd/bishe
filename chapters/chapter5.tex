...
为了应对真实数据高噪声带来的优化难题，本节对微调超参数进行了重新设计。首先，为防止在微调初期破坏预训练阶段建立的特征表示，模型的基础学习率被大幅降低至 $2 \times 10^{-6}$。其次，为了让模型逐步适应高噪声的数据分布，学习率调度策略采用了线性预热结合余弦衰减，其中预热比例设为0.02，使得训练前期的梯度更新更为平缓。此外，针对可能出现的梯度爆炸现象，最大梯度裁剪范数被限制在0.5，同时配合0.001的权重衰减系数以防止过拟合。在上述配置下，模型在真实数据集上训练了4000步。
