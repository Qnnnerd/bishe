...
图~\ref{fig:action_distribution_placeholder}~展示了自建数据集与LIBERO数据集的动作分布对比。在真实机器人的遥操作采集中，为了保证设备与环境的安全性，操作者通常会采取更为保守的控制策略。真实数据集中的每组动作序列因此包含更多的执行步数，单步动作的位姿变化量也小于仿真环境中的动作幅度。这也是本章在真实场景下进行专项微调的动因之一。

\begin{figure}[htbp]
  \centering
  % 占位图：自建数据集的动作分布和libero动作分布对比图
  \includegraphics[width=0.8\linewidth]{example-image-c}
  \caption{自建数据集与LIBERO数据集动作分布对比}
  \label{fig:action_distribution_placeholder}
\end{figure}
