...
预训练模型在LIBERO等仿真环境中学习到的动作分布，往往难以直接迁移至真实物理世界。仿真环境与真实物理世界的差异主要集中在视觉层面，包括光照条件的变化、摄像头视角的偏移以及物理接触产生的视觉噪声；而语言指令所描述的任务语义在两个域之间差别不大。因此，本节在微调时确立了参数冻结策略：冻结LLM骨干的绝大部分参数，仅保留最后一层参与训练，同时视觉编码器和历史存储池相关模块全部开放更新。这一策略在两个层面上改善了训练质量：可训练参数量的减少缩短了每步的反向传播路径，训练速度随之提升；LLM骨干在预训练阶段建立的语义表征不会因梯度反传被破坏，为后续的稳定训练奠定了基础。
...
系统的整体数据流转与节点配合机制如图~\ref{fig:ros_dataflow}~所示。在每次控制循环中，本地控制节点（\texttt{action\_control.py}）首先通过USB相机采集RGB图像，并将其与当前任务的语言指令、唯一的会话标识一同封装为HTTP请求发送至远端Flask推理服务。推理服务在接收到请求后，首先进行会话校验，确保当前图像与历史特征缓存的时序对齐；随后将图像输入视觉语言动作模型，生成包含15步末端位姿差分的动作序列，并将其封装为JSON格式的HTTP响应返回给控制节点。
...
在获取到物理尺度的动作序列后，系统必须将其转换为关节空间的轨迹才能下发给底层控制器。为此，控制节点通过订阅 \texttt{/joint\_states} 话题，实时获取UR5e机械臂当前的6个真实关节角度。随后，系统利用DH参数进行正运动学计算，得到当前末端执行器在基坐标系下的位姿矩阵。在此基础上，将模型输出的平移与旋转差分叠加至当前位姿，生成目标位姿矩阵。最后，调用 \texttt{trac\_ik} 运动学求解器计算出对应的目标关节角。
...
经过上述安全校验后的目标关节角序列，会被封装为 \texttt{FollowJointTrajectoryGoal} 消息，并通过 \texttt{/follow\_joint\_trajectory} 动作接口发送给UR驱动器。同时，夹爪的状态指令会同步发布至 \texttt{/gripper/ctrl} 话题。底层控制器在接收到轨迹指令后，驱动各关节电机完成物理执行，并将更新后的真实状态再次发布至 \texttt{/joint\_states}，从而形成了一个完整、安全的"感知-推理-执行-反馈"闭环控制回路。
