...
预训练模型在LIBERO等仿真环境中学习到的动作分布，往往难以直接迁移至真实物理世界。其根本原因在于，仿真环境中的动作通常是平滑且理想化的，而真实场景下的遥操作数据天然携带着较高的人为噪声。操作者在执行任务时，由于视角受限或手部抖动，会频繁进行微小的位姿调整；同时，真实机械臂在接触桌面或物体时存在复杂的摩擦与碰撞反馈，导致采集到的动作序列在局部呈现出高频的振荡特征。这种数据分布的根本差异，使得模型在真实数据上微调时，如果直接沿用仿真阶段的超参数，会导致梯度更新方向在噪声样本中来回拉扯，表现为训练损失剧烈波动，模型难以收敛到稳定的局部最优解。

仿真环境与真实物理世界的差异主要集中在视觉层面，包括光照条件的变化、摄像头视角的偏移以及物理接触产生的视觉噪声；语言指令所描述的任务语义在两个域之间差别不大。因此，本节微调时冻结了LLM骨干的绝大部分参数，仅保留最后一层参与训练，视觉编码器和历史存储池相关模块则全部开放更新。这一策略在两个层面上改善了训练质量：可训练参数量的减少缩短了每步的反向传播路径，训练速度随之提升；LLM骨干在预训练阶段建立的语义表征不会因梯度反传被破坏，训练过程的稳定性因此得到保障。

在冻结策略确定后，本节针对真实数据的高噪声特性，对微调超参数进行了重新设计。基础学习率设为 $2 \times 10^{-6}$，以防止微调初期破坏预训练阶段建立的特征表示；学习率调度采用线性预热结合余弦衰减，预热比例0.02，使训练前期的梯度更新趋于平缓。为抑制可能出现的梯度爆炸，最大梯度裁剪范数限制在0.5，同时配合权重衰减系数0.001以防止过拟合。在上述配置下，模型在真实数据集上训练了4000步。
