...
经过上述安全校验后的目标关节角序列，会被封装为 \texttt{FollowJointTrajectoryGoal} 消息，并通过 \texttt{/follow\_joint\_trajectory} 动作接口发送给UR驱动器。同时，夹爪的状态指令会同步发布至 \texttt{/gripper/ctrl} 话题。底层控制器在接收到轨迹指令后，驱动各关节电机完成物理执行，并将更新后的真实状态再次发布至 \texttt{/joint\_states}，从而形成了一个完整、安全的感知推理执行反馈闭环控制回路。
