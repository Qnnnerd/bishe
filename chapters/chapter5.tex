...
预训练模型在LIBERO等仿真环境中学习到的动作分布，往往难以直接迁移至真实物理世界。其根本原因在于，仿真环境中的动作通常是平滑且理想化的，而真实场景下的遥操作数据天然携带着较高的人为噪声。操作者在执行任务时，由于视角受限或手部抖动，会频繁进行微小的位姿调整；同时，真实机械臂在接触桌面或物体时存在复杂的摩擦与碰撞反馈，导致采集到的动作序列在局部呈现出高频的振荡特征。这种数据分布的根本差异，使得模型在真实数据上微调时，如果直接沿用仿真阶段的超参数，会导致梯度更新方向在噪声样本中来回拉扯，表现为训练损失（Loss）剧烈波动，模型难以收敛到稳定的局部最优解。

为了应对真实数据高噪声带来的优化难题，本节对微调超参数进行了重新设计。首先，为防止在微调初期破坏预训练阶段建立的特征表示，模型的基础学习率被大幅降低至 $2 \times 10^{-6}$。其次，为了让模型逐步适应高噪声的数据分布，学习率调度策略采用了线性预热结合余弦衰减（linear-warmup+cosine-decay），其中预热比例设为0.02，使得训练前期的梯度更新更为平缓。此外，针对可能出现的梯度爆炸现象，最大梯度裁剪范数被严格限制在0.5，同时配合0.001的权重衰减系数以防止过拟合。在上述配置下，模型在真实数据集上训练了20000步。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{figures/content/loss_comparison_placeholder.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{figures/content/loss_sim_vs_real_placeholder.png}
...
微调完成后，模型需要被部署到实际控制系统中以进行在线推理。然而，视觉语言动作模型的参数量达到数十亿级别，难以在计算资源受限的边缘设备或工控机上进行本地推理；同时，模型加载与初始化的时间成本极高，无法满足机器人控制系统对实时性的严苛要求。

针对这一问题，本节采用计算与控制分离的部署架构。将模型作为常驻服务部署在具备多张高性能GPU的远端服务器上，并通过Flask框架封装为HTTP接口。控制端仅负责轻量级的图像采集与指令发送，通过网络请求获取推理结果。这种架构避免了每次执行任务时的模型加载开销，保障了推理服务的高可用性。

在服务接口的具体设计上，为了适应真实场景的任务需求，本节对动作分块（Action Chunking）和时序缓存的参数进行了针对性调整。由于真实采集的动作步幅较小，单次推理若只返回一步动作，将导致机械臂执行过程出现严重的卡顿与延迟。因此，部署时将动作分块长度设置为15步，即单次网络请求返回一段连续的15步动作序列，从而保证了物理执行的连贯性。同时，考虑到真实长时效任务的耗时远超仿真环境，系统将历史特征的缓存长度设定为256帧，以覆盖更长的时间跨度，避免任务后期的状态遗忘。

此外，在长时效任务的实际执行中，网络延迟或机械臂执行阻塞可能导致控制端的图像采集帧率与云端推理服务的接收频率出现时序错位。这种错位会导致模型基于旧的图像特征预测当前的动作，进而引发不可逆的执行错误。为了解决这一隐患，推理服务接口引入了会话校验（Session ID）机制。控制端在发起请求时必须附带当前任务的唯一会话标识与递增的请求序号；云端服务通过校验这些标识，确保当前接收到的图像、历史缓存特征与待生成的动作序列在时序上严格对齐。通过上述部署策略，系统实现了对大参数量视觉语言动作模型的在线推理支持。

\subsection{基于ROS的机械臂动作控制系统}
\label{subsec:ros_control_system}

远端推理服务解决了模型的计算与存储难题，但模型输出的动作序列仅为数值化的位姿差分，无法直接驱动物理机械臂。为了实现从高层语义动作到低层电机转角的安全转换，本节基于ROS（Robot Operating System）框架，设计并实现了本地的机械臂动作控制系统。该系统不仅承担着数据路由与协议转换的任务，更通过逆运动学求解与多层安全限幅机制，在物理执行层面保障了人机交互的安全性。

系统的整体数据流转与节点配合机制如图~\ref{fig:ros_dataflow}~所示。在每次控制循环中，本地控制节点（\texttt{action\_control.py}）首先通过USB相机采集RGB图像，并将其与当前任务的语言指令、唯一的会话标识（Session ID）一同封装为HTTP请求发送至远端Flask推理服务。由于模型在训练阶段采用了数据归一化处理，推理服务在生成预测后，会直接在内部进行反归一化操作。因此，控制节点接收到的动作分块，已经是物理尺度下的15步连续动作序列，其中前6维代表末端执行器在平移与旋转方向上的差分（平移尺度因子为0.05，旋转尺度因子为0.2），第7维代表夹爪的开合状态。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{figures/content/ros_dataflow.png}
\caption{基于ROS的机械臂动作推理与控制数据流转图}
\label{fig:ros_dataflow}
\end{figure}

在获取到物理尺度的动作序列后，系统必须将其转换为关节空间的轨迹才能下发给底层控制器。为此，控制节点通过订阅 \texttt{/joint\_states} 话题，实时获取UR5e机械臂当前的6个真实关节角度。随后，系统利用DH参数进行正运动学（FK）计算，得到当前末端执行器在基坐标系下的位姿矩阵。在此基础上，将模型输出的平移与旋转差分叠加至当前位姿，生成目标位姿矩阵。最后，调用 \texttt{trac\_ik} 运动学求解器计算出对应的目标关节角。

然而，视觉语言模型在推理长时效任务时，由于累积误差或对未见场景的误判，偶尔会生成幅度过大或方向突变的错误动作。如果将这些动作直接交由逆运动学求解并执行，极易导致机械臂与桌面或周围环境发生剧烈碰撞。针对这一隐患，本节在控制节点中引入了三层严格的安全限幅机制。首先是末端位移限幅，将单步平移的最大距离限制在0.03米以内；其次是旋转限幅，将单步姿态变化的最大角度限制在0.15弧度以内；最后是关节跳变检测，当逆运动学求解出的目标关节角与当前关节角之差超过0.5弧度时，系统将主动丢弃该步动作并保持当前位姿。

经过上述安全校验后的目标关节角序列，会被封装为 \texttt{FollowJointTrajectoryGoal} 消息，并通过 \texttt{/follow\_joint\_trajectory} 动作（Action）接口发送给UR驱动器。同时，夹爪的状态指令会同步发布至 \texttt{/gripper/ctrl} 话题。底层控制器在接收到轨迹指令后，驱动各关节电机完成物理执行，并将更新后的真实状态再次发布至 \texttt{/joint\_states}，从而形成了一个完整、安全的"感知-推理-执行-反馈"闭环控制回路。
...
实验共设计了三类复杂度递增的真实操作任务：第一类是单步抓取任务（如抓取桌面上的红色方块），用于评估模型的基础动作精度；第二类是抽屉操作任务（如拉开顶层抽屉），要求模型能够理解铰链机构的运动约束；第三类是长时效组合任务（如打开抽屉并将物品放入），要求模型具备跨阶段的状态记忆能力。每类任务在相同的初始场景下各进行20次测试，以成功完成语言指令指定的全部操作为成功标准。图~\ref{fig:real_success_traj}~展示了TemporalCacheVLA模型在三类任务中的成功执行轨迹。
...
对比实验的具体成功率如表~\ref{tab:real_world_results}~所示。从结果可以看出，在单步抓取任务中，两者的成功率差距较小（85对75），这表明在无需历史上下文的简单任务中，OpenVLA本身已经具备了较好的动作生成能力。然而，随着任务复杂度的提升，特别是在需要跨阶段状态记忆的长时效组合任务中，OpenVLA的成功率急剧下降至30，而TemporalCacheVLA依然保持了65的成功率。这一差异表明，时序缓存机制在真实长时效任务中同样能够发挥作用。
...
需要指出的是，即使是表现较好的TemporalCacheVLA模型，其在真实场景下的平均成功率（73.3）也低于第四章在LIBERO仿真环境中的表现（94.9）。这种性能衰减主要源于真实物理世界的复杂性：一方面，环境光照的微小变化与相机的细微视角偏移都会对视觉特征的提取造成干扰；另一方面，夹爪的物理磨损与被操作物体的表面摩擦系数在每次测试中都略有不同，这些在训练数据中未能充分覆盖的物理因素增加了动作执行的失败概率。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{figures/content/real_trajectory_failure_placeholder.png}
