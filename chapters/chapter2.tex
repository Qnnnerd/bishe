...
视觉语言动作模型（Vision-Language-Action Models, VLA）是近年来在具身智能领域兴起的一种新型策略学习架构，其核心目标是将大规模预训练模型中蕴含的丰富语义知识与逻辑推理能力，迁移至物理世界的机器人底层控制任务中。传统的机器人操作策略通常将视觉感知与动作生成作为两个独立的模块进行设计，或依赖于从零开始训练的端到端小规模网络，这导致其在面对未见物体或复杂指令时泛化能力受限。视觉语言动作模型通过打破这种模块隔离，将机器人的“感知”、“理解”与“执行”整合在一个统一的网络架构中。
