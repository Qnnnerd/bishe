...
模仿学习旨在让机械臂通过学习人类示范数据来获取有效的行为策略，从而使其能够快速掌握复杂的任务技能，有效规避了强化学习中通常需要的大量试错过程。模仿学习的核心思想是利用专家提供的状态与动作数据作为监督信号，训练智能体的策略模型，使其在面对相似状态时能够输出与专家一致的动作。

在模仿学习的理论框架中，系统通常被建模为马尔可夫决策过程。给定状态空间 $\mathcal{S}$ 和动作空间 $\mathcal{A}$，专家的策略可以表示为一个条件概率分布 $\pi_E(a|s)$。在训练过程中，算法会接收一组由专家提供的演示轨迹集合 $\mathcal{D} = \{\tau_1, \tau_2, \dots, \tau_N\}$，其中每条轨迹 $\tau_i$ 包含了从初始状态到终止状态的一系列状态与动作的映射，即 $\tau_i = \{(s_1, a_1), (s_2, a_2), \dots, (s_T, a_T)\}$。

传统行为克隆是模仿学习中最基础且应用最广泛的方法。它将策略学习直接转化为一个监督学习问题，其目标是寻找一个参数化策略 $\pi_\theta(a|s)$，使得在专家演示数据分布下，该策略输出的动作与专家动作之间的误差最小化。具体而言，行为克隆通过最小化负对数似然损失或均方误差损失来优化策略参数 $\theta$。以确定性策略为例，其优化目标可表示为：

\begin{equation}
  \theta^* = \arg\min_\theta \mathbb{E}_{(s,a) \sim \mathcal{D}} \left[ \mathcal{L}(a, \pi_\theta(s)) \right]
\end{equation}

其中，$\mathcal{L}$ 是损失函数，例如均方误差 $\mathcal{L}(a, \hat{a}) = \|a - \hat{a}\|^2$。尽管行为克隆方法简单直接，但它在实际应用中存在明显的局限性。首先，该方法高度依赖于示范者的动作序列，而在现实场景中，人类在演示时往往只能展示动作的最终效果，难以获取其内部真实的控制信号，这在一定程度上限制了行为克隆的应用范围。其次，行为克隆极易受到协变量转移的影响。在实际执行过程中，由于模型预测的微小误差会随时间累积，导致机械臂遇到的状态分布逐渐偏离训练时的示范数据分布。这种分布的偏移会使得模型在未见过的状态下性能急剧下降。此外，传统的行为克隆通常只能学习单一的行为策略，难以适应复杂多变的环境与多样化的任务需求，从而降低了机械臂执行任务的灵活性与成功率。

为了解决传统行为克隆方法的局限性，研究者们不断探索新的网络架构与算法模型，以提升模仿学习在复杂场景下的表现，其中基于 Transformer 架构和扩散模型的方法在近期展现出了显著的优势。

\subsection{典型模仿学习算法}
\label{subsec:typical_il_algorithms}

在众多模仿学习算法中，动作分块 Transformer 算法 \cite{zhao2023learning} 和扩散策略算法 \cite{chi2023diffusion} 是近期在机器人操作领域表现突出的两种典型方法。

动作分块 Transformer 算法主要用于解决精细操作任务中的模仿学习问题。该算法基于 Transformer 架构构建了条件变分自编码器的编码器和解码器，用于对示教数据进行建模。在特征处理方面，编码器负责高效提取输入信息的关键特征，而解码器则先借助残差网络对图像进行编码，随后通过 Transformer 编码器将多模态特征进行深度融合，最终生成连贯且流畅的动作序列。

针对模仿学习中常见的误差累积和人类演示非平稳问题，该算法采用了动作分块策略。这一策略将连续的动作序列分割为多个相对独立的动作组块，从而有效减少了误差在动作执行过程中的传递与积累。在面对高精度控制和闭环反馈的精细操作任务时，策略会预测未来 $k$ 个时间步的动作，使得任务的有效视野降低 $k$ 倍，以此来缓解误差累积的问题。此外，为了提升策略的平滑性，算法还引入了时间集成机制。在每个时间步推理动作序列时，会生成不同时刻的动作组块并按时序排列，使得不同动作组块相互重叠。在任一时间步生成多个预测动作后，采用指数加权平均的方式组合这些预测。设权重系数为 $w_i = \exp(-m * i)$，其中 $w_0$ 是初始动作的权重，$m$ 为衰减因子，通过这种加权组合进一步优化了动作生成的质量。
...
尽管动作分块 Transformer 算法在精细操作任务中成效显著，但在执行复杂手部动作和处理高维动作空间时仍存在不足。相比之下，扩散策略算法在处理复杂动作分布时能够更精准地生成合适策略，并高效探索高维动作空间以寻找最优解。此外，其出色的训练稳定性保障了算法在各类场景下的可靠运行。

扩散策略算法基于去噪扩散概率模型，以随机朗之万动力学为理论基础，将动作序列的生成视为一个去噪过程。与直接将状态映射到动作的显式策略，以及通过能量函数定义动作分布但训练不稳定的隐式策略不同，扩散策略不直接输出动作，而是基于视觉观察推断动作分数的梯度，并通过迭代步骤逐步优化动作。这种方式使得算法能够自然且精确地表达多模态动作分布，灵活应对机械臂操作中的多种动作选择，并且能够联合推断未来动作序列以避免短视规划。

在具体原理方面，扩散策略的输出生成过程被建模为逐步去噪的过程。在第 $k$ 次去噪迭代中，动作的更新公式为：

\begin{equation}
  x^{k-1} = x^k - \alpha \nabla_{x^k} \log p(x^k) + \gamma \epsilon
\end{equation}

为了适应机械臂任务，扩散策略在视觉观察条件设定方面进行了关键改进，使用扩散模型近似条件分布，以加快扩散过程并提高动作生成准确性。给定时间步 $t$ 的观察数据 $O_t$，预测动作序列 $A_t$ 的去噪过程可表示为：

\begin{equation}
  A_t^{k-1} = A_t^k - \alpha \epsilon_\theta(A_t^k, O_t, k) + \gamma \mathcal{N}(0, \sigma^2 I)
\end{equation}

其中，$\epsilon_\theta$ 是噪声预测网络，$\alpha$、$\gamma$ 和 $\sigma$ 是与迭代步骤 $k$ 相关的噪声调度参数。训练时，通过最小化均方误差损失函数来优化噪声预测网络：

\begin{equation}
  \mathcal{L}_{\text{MSE}} = \mathbb{E}_{k, A_t, \epsilon} \left[ \|\epsilon - \epsilon_\theta(A_t^k, O_t, k)\|^2 \right]
\end{equation}
...
在闭环动作序列预测上，扩散策略将动作序列预测能力与滚动时域控制相结合。在时间步 $t$，模型以最近 $T_o$ 步的观察数据为输入，预测 $T_p$ 步的动作序列，其中前 $T_a$ 步的动作会被直接执行在机械臂上而无需重新规划。这种设计有效平衡了长期规划与实时响应。在网络架构设计上，扩散策略可以采用卷积神经网络或 Transformer 架构进行构建。通过采用残差网络作为视觉编码器，并结合空间 Softmax 池化和组归一化等技术，模型有效提升了目标定位的精准度和训练的稳定性，从而在机器人动作生成方面表现出优异的性能。
