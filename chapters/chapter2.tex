...
视觉语言动作模型（Vision-Language-Action Models, VLA）是近年来在具身智能领域兴起的一种新型策略学习架构，其核心目标是将大规模预训练模型中蕴含的丰富语义知识与逻辑推理能力，迁移至物理世界的机器人底层控制任务中。本节将从相关理论基础出发，系统阐述视觉语言动作模型的核心原理、多模态融合机制以及动作生成的理论范式。

\subsection{视觉语言动作模型原理}
\label{subsec:vla_principles}

传统的机器人操作策略通常将视觉感知与动作生成作为两个独立的模块进行设计，或依赖于从零开始训练的端到端小规模网络，这导致其在面对未见物体或复杂指令时泛化能力受限。视觉语言动作模型通过打破这种模块隔离，将机器人的“感知”、“理解”与“执行”整合在一个统一的网络架构中。

在形式化表示上，视觉语言动作模型将机器人的操作任务转化为一个多模态条件序列生成问题。给定当前时刻的视觉观测图像 $I$ 和人类操作者提供的自然语言指令 $L$，模型的目标是输出一个能够完成该指令的动作序列 $A$。其优化目标可以表示为最大化在给定视觉和语言条件下的动作序列对数似然：

\begin{equation}
  \theta^* = \arg\max_\theta \mathbb{E}_{(I, L, A) \sim \mathcal{D}} \left[ \log P_\theta(A | I, L) \right]
\end{equation}

为了将连续的物理动作空间与离散的自然语言空间统一在同一架构下，VLA 普遍采用动作离散化（Action Discretization）策略。具体而言，机械臂末端执行器的连续六自由度位姿变化和夹爪的开合状态，被按照一定的精度划分并映射到与语言词表相同维度的离散标记（Action Tokens）空间中。通过这种方式，视觉图像特征、语言指令和机器人动作被统一表示为离散的 Token 序列，从而可以直接输入到基于 Transformer 架构的大语言模型中进行联合训练与推理。

\subsection{多模态融合与动作生成机制}
\label{subsec:multimodal_fusion_and_generation}

在统一了输入输出表示后，视觉语言动作模型的核心在于如何有效融合多模态特征并生成平滑可靠的动作指令。这一过程通常包含视觉特征提取、跨模态对齐以及自回归动作生成三个关键环节。

在视觉特征提取阶段，为了应对非结构化环境中的复杂感知需求，现代 VLA 架构（如 OpenVLA \cite{kim2024openvla}）倾向于采用多通道视觉编码器融合的策略。通过结合擅长提取语义对齐特征的对比语言图像预训练模型（如 SigLIP）和擅长捕捉空间几何与物体结构信息的自监督视觉模型（如 DINOv2），系统能够获得兼具高层语义与底层物理属性的综合视觉表征。这些视觉特征随后被投影到语言模型的嵌入空间中，转化为视觉标记（Visual Tokens）。

在跨模态对齐与动作生成阶段，视觉标记与语言指令标记（Language Tokens）被拼接为统一的上下文序列，输入到大规模 Transformer 解码器中。模型通过自回归（Autoregressive）的方式，基于历史上下文逐个预测出代表未来动作的离散标记：

\begin{equation}
  P_\theta(A | I, L) = \prod_{t=1}^{T_a} P_\theta(a_t | I, L, a_{<t})
\end{equation}

其中 $a_t$ 为第 $t$ 步的动作标记，$T_a$ 为预测的动作步数。预测完成的离散标记最后通过反量化（Dequantization）操作，还原为连续的物理控制信号并下发至机械臂底层控制器。

这种将动作生成视作特殊“语言”输出的范式，使得 VLA 能够直接继承预训练大模型在互联网海量数据中学习到的常识推理能力。然而，受限于自回归生成的计算成本和模型架构的单帧输入特性，纯粹的 VLA 在处理具有长时序依赖的复杂操作任务时，仍面临由于缺乏历史上下文而导致的动作不连贯与状态混淆等问题。这为后续研究中引入时序缓存机制与扩散动作生成模块提供了理论动机与改进空间。
