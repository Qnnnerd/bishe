...
视觉语言动作模型（Vision-Language-Action Models, VLA）是近年来在具身智能领域兴起的一种新型策略学习架构。它通过将预训练的视觉语言模型（VLM）与机器人动作生成模块进行深度融合，旨在将大模型在互联网规模数据上学习到的丰富语义知识与逻辑推理能力，直接迁移并应用于物理世界的机器人底层控制任务中。本节将介绍视觉语言动作模型的基本原理、典型架构演进，并重点探讨以 OpenVLA 为代表的开源端到端框架的技术特点。

\subsection{视觉语言动作模型原理}
\label{subsec:vla_principles}

传统的机器人操作策略通常将视觉感知与动作生成作为两个独立的模块进行设计，或者依赖于从零开始训练的端到端小规模网络。这些方法在处理特定任务时表现良好，但在面对未见过的物体、指令或复杂环境时，泛化能力往往受限。视觉语言动作模型的核心思想在于打破这种隔离，将机器人的“看”、“想”与“做”整合在一个统一的网络架构中。

在形式化表示上，视觉语言动作模型将机器人的操作任务转化为一个条件序列生成问题。给定当前时刻的视觉观测图像 $I$ 和人类操作者提供的自然语言指令 $L$，模型的目标是输出一个能够完成该指令的动作序列 $A$。与纯粹的语言模型或视觉语言模型不同，VLA 的输出不仅包含文本描述，还包含了机器人末端执行器的六自由度位姿变化、夹爪的开合状态等底层控制信号。为了实现这一目标，VLA 通常采用离散化动作标记（Action Tokens）的策略，即将连续的机器人动作空间映射到与语言词表相同维度的离散标记空间中。这样一来，视觉图像、语言指令和机器人动作就可以被统一编码，并输入到基于 Transformer 架构的大模型中进行联合训练与推理。

\subsection{典型视觉语言动作模型架构}
\label{subsec:vla_architectures}

视觉语言动作模型的发展经历了从模块化组合到端到端生成的演进过程。早期的研究主要利用大语言模型作为高级任务规划器，将复杂的自然语言指令分解为一系列机器人可执行的子任务，再由底层的传统控制算法完成具体动作。这种方法虽然提升了机器人的语义理解能力，但由于高层规划与底层控制之间的分离，模型无法进行端到端的优化，难以处理需要精细视觉反馈的复杂操作任务。

随后，以 RT-1 \cite{brohan2022rt1} 和 RT-2 \cite{brohan2023rt2} 为代表的端到端视觉语言动作模型相继被提出。RT-1 提出了一种基于 Transformer 的架构，将图像序列和语言指令作为输入，直接输出离散化的机器人动作。RT-2 则进一步将机器人动作编码为文本标记，直接复用了大规模预训练的视觉语言模型（如 PaLI-X 和 PaLM-E）的权重。这种直接将动作序列视作一种特殊“语言”进行输出的范式，使得模型能够继承互联网数据中的常识推理能力，并在真实机器人平台上展现出零样本泛化能力。然而，这些闭源模型由于缺乏开放的权重和训练细节，限制了学术界的进一步研究与改进。

\subsection{基于开源大模型的 OpenVLA 框架}
\label{subsec:openvla_framework}

为了推动通用机器人模型的普及与发展，研究人员基于开源数据集 Open X-Embodiment \cite{open2023x} 提出了开源视觉语言动作模型 OpenVLA \cite{kim2024openvla}。OpenVLA 采用了一种高度集成且参数高效的架构设计，其核心优势在于对预训练视觉编码器与开源大语言模型的深度整合。

在视觉感知层面，OpenVLA 摒弃了单一的视觉特征提取器，转而采用融合了 SigLIP \cite{zhai2023sigmoid} 和 DINOv2 \cite{oquab2023dinov2} 两种预训练模型的双通道视觉编码器。SigLIP 擅长提取与自然语言高度对齐的语义特征，而 DINOv2 则能够捕捉图像中丰富的空间几何与物体结构信息。这种互补的视觉特征表示，为模型在复杂非结构化环境中的精准操作提供了坚实的感知基础。

在语言理解与动作生成层面，OpenVLA 基于开源的 Llama-2 \cite{touvron2023llama} 大语言模型构建。通过在海量机器人操作轨迹数据上进行微调，模型学习到了从多模态视觉特征与语言指令到离散动作标记的映射关系。具体而言，输入的图像经过视觉编码器处理后被转换为视觉标记（Visual Tokens），随后与语言指令标记（Language Tokens）拼接，共同输入到 Llama-2 的 Transformer 解码器中。模型通过自回归的方式，逐个预测出代表机器人动作的离散标记，最后通过反量化操作将其还原为连续的物理控制信号。

OpenVLA 的提出证明了基于开源大模型构建通用视觉语言动作模型的可行性。然而，该框架主要依赖当前单帧观测进行自回归决策，在处理具有长时序依赖的复杂操作任务时，仍面临由于缺乏历史上下文而导致的动作不连贯与状态混淆等问题。这一局限性也为后续引入时序缓存机制与扩散动作生成模块提供了理论动机。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/content/[此处需要补充具体数据/文献/细节]}
  \caption{典型视觉语言动作模型架构示意图}
  \label{fig:vla_architecture_overview}
\end{figure}
