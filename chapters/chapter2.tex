...
此外，为解决开环执行动作块可能导致的机械臂动作不连贯与抖动问题，ACT 引入了时间集成（Temporal Ensembling）机制。在任意物理运行时间步 $t$，系统内部会同时存在来自过去 $k$ 个不同时刻对当前时间步的动作预测值。设 $\hat{a}_t^{(t-i)}$ 表示系统在过去时刻 $t-i$ 处所生成的针对当前时刻 $t$ 的目标动作预测，动作集成机制通过指数加权移动平均的方式对这些预测值进行融合：
...
尽管 ACT 算法在精细操作任务中成效显著，但其通过 CVAE 的高斯先验强制规范隐变量的方式，在处理存在严苛物理接触约束的多模态分布时仍显吃力。相比之下，扩散策略算法（Diffusion Policy）\cite{chi2023diffusion} 创造性地将机械臂动作序列的生成过程重构为去噪扩散概率模型（DDPM）的条件去噪演化过程，展现出了对复杂动作空间无与伦比的建模精度与超强的训练稳定性。
...
视觉语言动作模型是近年来具身智能领域发展出的一种新型策略架构。其核心思想是将大规模预训练模型中蕴含的语义知识与逻辑推理能力，迁移至物理世界的机器人底层控制任务中\cite{kawaharazuka2025vision}。为了实现这一目标，需要将视觉感知、语言理解与动作生成整合在统一的数学框架内。本节将从相关理论基础出发，系统阐述大语言模型的自回归生成原理，以及视觉特征提取、多模态融合与动作离散化生成的具体实现方式。
...
随后，这些视觉标记 $\mathbf{v}_{\text{proj}}$ 与人类操作者提供的自然语言指令标记 $\mathbf{e}_L$ 在序列维度上进行拼接，构成一个统一的多模态上下文序列 $[\mathbf{v}_{\text{proj}}; \mathbf{e}_L]$，送入大语言模型的 Transformer 解码器中进行联合处理\cite{brohan2023rt2}。

为了将连续的物理动作空间与离散的自然语言空间统一在同一网络架构下，视觉语言动作模型普遍采用动作离散化策略\cite{shridhar2023perceiver}。机械臂末端执行器的连续多自由度动作向量被独立地离散化。具体做法是将每个动作维度在训练数据分布内的有效区间划分为 $B$ 个均匀的离散区间（Bin）。对于动作向量中的第 $i$ 个连续值 $a_i$，其对应的离散标记索引 $c_i \in \{0, 1, \dots, B-1\}$ 可通过下式计算：
