...
视觉语言动作模型是近年来具身智能领域发展出的一种新型策略架构。其核心思想是将大规模预训练模型中蕴含的语义知识与逻辑推理能力，迁移至物理世界的机器人底层控制任务中。为了实现这一目标，需要将视觉感知、语言理解与动作生成整合在统一的数学框架内。本节将从相关理论基础出发，系统阐述大语言模型的自回归生成原理、视觉特征提取与多模态融合机制，以及动作离散化与序列生成的具体实现方式。

\subsection{大语言模型与自回归生成}
\label{subsec:llm_autoregressive}

大语言模型是进行高层语义理解与决策推理的核心组件。其底层架构通常采用基于自注意力机制的 Transformer 解码器。在自然语言处理领域，模型通过在海量文本语料上进行无监督的下一个词预测训练，学习到了语言的语法结构、上下文依赖关系以及广泛的常识知识。
...
在机器人操作任务中，仅有语言指令不足以指导具体的物理动作，系统必须具备对当前环境状态的精准视觉感知能力。视觉编码器负责将高维的原始图像数据降维并提取为结构化的特征表示。为了应对非结构化环境中的复杂感知需求，通常采用多通道视觉编码器融合的策略。

一方面，系统利用对比语言图像预训练模型来提取语义对齐特征。这类模型在训练时通过最大化成对图像与文本描述的相似度，使得提取出的视觉特征能够与自然语言指令在同一嵌入空间中实现对齐，从而帮助机器人理解图像中物体的语义类别与属性。另一方面，系统引入自监督视觉模型来补充空间几何与物体结构信息。这类模型通过掩码图像预测等自监督任务，能够捕捉到图像中细粒度的边缘、深度以及物体的局部特征，这对于需要高精度物理接触的操作任务至关重要。

多模态融合的关键在于将提取到的视觉特征映射到语言模型的输入空间中。具体而言，综合后的视觉特征向量会通过一个多层感知机或投影矩阵，被转换为与语言词元维度一致的视觉标记。随后，这些视觉标记与人类操作者提供的自然语言指令标记在序列维度上进行拼接，构成一个统一的多模态上下文序列，送入大语言模型的 Transformer 解码器中进行联合处理。

\subsection{动作离散化与序列生成}
\label{subsec:action_discretization}

为了将连续的物理动作空间与离散的自然语言空间统一在同一网络架构下，视觉语言动作模型普遍采用动作离散化策略。机械臂末端执行器的连续六自由度位姿变化以及夹爪的开合状态，被按照一定的精度范围划分为多个离散的区间。每个区间被赋予一个唯一的整数索引，从而将连续的动作向量映射为与语言词表相同维度的离散动作标记。
...
这种将动作生成视作特殊语言输出的机制，使得模型能够直接继承预训练模型在海量数据中学习到的推理能力。然而，受限于自回归生成的计算成本和模型架构的单帧输入特性，这类模型在处理具有长时序依赖的复杂操作任务时，仍面临由于缺乏历史上下文而导致的动作不连贯与状态混淆等问题。这为后续研究中引入时序缓存机制与扩散动作生成模块提供了理论动机与改进空间。
