...
本章旨在为后文提出的机器人自主操作算法提供必要的理论支撑与技术背景。内容将围绕机器人模仿学习这一核心技术路线展开，系统性地介绍其研究所需的前置知识。首先，将对机械臂运动学与控制的基本原理进行阐述，重点介绍 Denavit-Hartenberg (D-H) 参数法在运动学建模中的应用，为后续的动作生成提供物理约束与数学基础。随后，将介绍模仿学习理论，探讨其基本原理以及当前典型的模仿学习算法。最后，将分别介绍视觉语言模型与扩散概率模型的核心思想与结构原理，它们共同构成了本文所提算法的基石。
...
Denavit-Hartenberg (D-H) 参数法是一种建立机械臂各连杆坐标系并描述其相对位姿的标准方法，如图\ref{fig:dh_parameters}所示，它仅用四个参数即可唯一确定两个相邻连杆坐标系之间的几何关系。这四个参数，即连杆长度 $a_{i-1}$、连杆扭转角 $\alpha_{i-1}$、连杆偏距 $d_i$ 和关节角 $\theta_i$，其定义如表\ref{tab:dh_parameters}所示。
...
模仿学习旨在通过观察专家演示来获取策略，使智能体能够执行复杂的任务。本节将详细探讨模仿学习的基本原理，并重点介绍当前典型的模仿学习算法，包括动作分块 Transformer（Action Chunking with Transformers, ACT）和扩散策略（Diffusion Policy）。

\subsection{模仿学习原理}
\label{subsec:il_principles}

模仿学习的核心思想是利用专家提供的状态-动作对作为监督信号，训练智能体的策略模型，使其在面对相似状态时能够输出与专家一致的动作。这一过程通常被建模为马尔可夫决策过程（Markov Decision Process, MDP）下的序列决策问题。在标准设定中，专家提供了一组演示轨迹集合 $\mathcal{D} = \{\tau_1, \tau_2, \dots, \tau_N\}$，其中每条轨迹 $\tau_i = \{(s_1, a_1), (s_2, a_2), \dots, (s_T, a_T)\}$ 包含了从初始状态到终止状态的一系列状态 $s_t$ 与动作 $a_t$ 的映射。

最基础的模仿学习方法是行为克隆（Behavioral Cloning, BC），它将策略学习转化为一个监督学习问题。在行为克隆中，智能体通过最小化策略输出动作与专家演示动作之间的误差来优化策略参数。尽管行为克隆在许多任务中取得了显著成果，但其面临着协变量偏移（Covariate Shift）的挑战。当智能体在执行过程中产生微小误差时，这些误差会随时间累积，导致智能体偏离专家演示的状态分布，进入未见过的状态空间，进而产生不可靠的动作输出。

为了缓解协变量偏移问题，研究者提出了多种改进方法。例如，逆强化学习（Inverse Reinforcement Learning, IRL）通过从专家演示中推导出一个奖励函数，然后利用强化学习算法基于该奖励函数训练策略。这种方法能够学习到任务的潜在目标，具有更好的泛化能力，但其计算成本较高，且奖励函数的推导往往存在歧义。生成对抗模仿学习（Generative Adversarial Imitation Learning, GAIL）则借鉴了生成对抗网络的思想，通过训练一个判别器来区分专家演示与智能体生成的轨迹，同时训练策略以欺骗判别器，从而直接学习到与专家相似的策略。

近年来，随着深度学习技术的发展，模仿学习在处理高维感知输入（如图像）和复杂动作输出方面取得了重大突破。特别是在机器人操作领域，基于视觉的模仿学习使得机器人能够从原始图像中直接学习端到端的控制策略，极大地降低了任务建模的复杂度。

\subsection{典型模仿学习算法}
\label{subsec:typical_il_algorithms}

在众多模仿学习算法中，动作分块 Transformer（ACT）和扩散策略（Diffusion Policy）是近期在机器人操作领域表现突出的两种典型方法。

动作分块 Transformer（ACT）旨在解决模仿学习中的复合误差和多模态分布问题。ACT 的核心创新在于将动作预测从单步动作扩展为多步动作序列（即“动作分块”）。通过预测未来的一个动作序列，智能体可以在执行过程中保持动作的连贯性，减少单步预测误差的累积。在网络结构上，ACT 采用 Transformer 架构来处理视觉输入和状态序列。此外，ACT 引入了条件变分自编码器（Conditional Variational Autoencoder, CVAE）来对专家演示中的多模态行为进行建模。在训练阶段，CVAE 的编码器将未来动作序列编码为一个隐变量，解码器则基于当前观测和隐变量重建动作序列；在推理阶段，智能体通过从先验分布中采样隐变量来生成不同的动作序列，从而能够应对具有多种可能解决方案的任务。
...
扩散策略（Diffusion Policy）则将扩散模型引入到机器人模仿学习中，将策略表示为一个条件去噪扩散过程。扩散模型在图像生成领域展现了强大的能力，能够生成高质量且具有多样性的样本。扩散策略利用这一特性，将动作生成视为从高斯噪声中逐步去噪的过程。具体而言，扩散策略在给定当前视觉观测和机器人状态的条件下，通过一个迭代去噪网络（如基于 U-Net 或 Transformer 的架构）逐步预测动作序列的梯度，最终生成符合专家演示分布的动作轨迹。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/content/[此处需要补充具体数据/文献/细节]}
  \caption{Diffusion Policy算法结构示意图}
  \label{fig:diffusion_policy_architecture}
\end{figure}

扩散策略的优势在于其能够自然地处理多模态动作分布，并且在复杂的高维动作空间中表现出优异的稳定性和表达能力。此外，扩散策略在处理闭环控制任务时，通过引入时间序列上的动作平滑机制，能够有效抑制高频抖动，生成平滑且鲁棒的控制指令。这两种算法在各自的理论框架下，为机器人模仿学习提供了新的解决思路，并在实际的复杂操作任务中取得了显著的成果。
