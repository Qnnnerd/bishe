...
视觉语言动作模型是近年来在具身智能领域兴起的一种新型策略学习架构。其核心目标是将大规模预训练语言模型中蕴含的丰富语义知识与逻辑推理能力，迁移至物理世界的机器人底层控制任务中。本节将从相关理论基础出发，系统阐述大语言模型的自回归生成原理、视觉编码器的特征提取机制，以及多模态融合与动作离散化的具体实现方式。

\subsection{大语言模型与自回归生成}
\label{subsec:llm_autoregressive}

大语言模型是视觉语言动作模型进行高层语义理解与决策推理的核心基石。其底层通常采用基于自注意力机制的 Transformer 解码器架构。在自然语言处理任务中，语言模型通过在海量文本语料上进行无监督的下一个词预测训练，学习到了语言的语法结构、上下文依赖关系以及广泛的世界知识。

从概率论视角来看，语言模型本质上是对离散序列的联合概率分布进行估计。给定一个由离散词元（Token）组成的序列 $X = (x_1, x_2, \dots, x_T)$，模型通过链式法则将其联合概率分解为各个词元在给定历史上下文条件下的条件概率乘积：

\begin{equation}
  P(X) = \prod_{t=1}^{T} P(x_t | x_1, x_2, \dots, x_{t-1})
\end{equation}

在推理阶段，模型采用自回归生成的方式，即根据当前已生成的序列内容，计算词表中所有候选词元的概率分布，并从中采样或选取概率最大的词元作为下一个输出，随后将其加入上下文序列中，循环往复直至生成终止符。这种自回归机制使得模型能够处理变长的序列输入与输出，并保持生成内容在逻辑上的连贯性。视觉语言动作模型正是利用了这一机制，将机器人动作序列的生成问题，转化为与文本生成在数学形式上完全等价的条件概率推断过程。

\subsection{视觉特征提取与多模态融合}
\label{subsec:vision_multimodal_fusion}

在机器人操作任务中，仅有语言指令不足以指导具体的物理动作，系统必须具备对当前环境状态的精准视觉感知能力。视觉编码器负责将高维的原始图像数据降维并提取为结构化的特征表示。为了应对非结构化环境中的复杂感知需求，现代视觉语言动作模型通常采用多通道视觉编码器融合的策略。

一方面，系统利用对比语言图像预训练模型（如 SigLIP）来提取语义对齐特征。这类模型在训练时通过最大化成对图像与文本描述的相似度，使得提取出的视觉特征能够与自然语言指令在同一嵌入空间中实现对齐，从而帮助机器人理解图像中物体的语义类别与属性。另一方面，系统引入自监督视觉模型（如 DINOv2）来补充空间几何与物体结构信息。这类模型通过掩码图像建模等自监督任务，能够捕捉到图像中细粒度的边缘、深度以及物体的局部特征，这对于需要高精度物理接触的操作任务至关重要。

多模态融合的关键在于将提取到的视觉特征映射到语言模型的输入空间中。具体而言，综合后的视觉特征向量会通过一个多层感知机或投影矩阵，被转换为与语言词元维度一致的视觉标记（Visual Tokens）。随后，这些视觉标记与人类操作者提供的自然语言指令标记（Language Tokens）在序列维度上进行拼接，构成一个统一的多模态上下文序列，送入大语言模型的 Transformer 解码器中进行联合处理。

\subsection{动作离散化与序列生成}
\label{subsec:action_discretization}

为了将连续的物理动作空间与离散的自然语言空间统一在同一网络架构下，视觉语言动作模型普遍采用动作离散化策略。机械臂末端执行器的连续六自由度位姿变化以及夹爪的开合状态，被按照一定的精度范围划分为多个离散的区间（Bins）。每个区间被赋予一个唯一的整数索引，从而将连续的动作向量映射为与语言词表相同维度的离散动作标记（Action Tokens）。
...
在实际生成过程中，模型基于历史上下文逐个预测出代表未来动作的离散标记。预测完成的离散标记序列最后通过反量化操作，即根据标记对应的索引查找其在连续空间中的中心值，还原为连续的物理控制信号，并下发至机械臂底层控制器执行。

这种将动作生成视作特殊语言输出的机制，使得模型能够直接继承预训练大模型在海量数据中学习到的常识推理能力。然而，受限于自回归生成的计算成本和模型架构的单帧输入特性，这类模型在处理具有长时序依赖的复杂操作任务时，仍面临由于缺乏历史上下文而导致的动作不连贯与状态混淆等问题。这为后续研究中引入时序缓存机制与扩散动作生成模块提供了理论动机与改进空间。
