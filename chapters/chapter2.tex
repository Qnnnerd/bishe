...
其中，$R \in SO(3)$ 是一个 $3 \times 3$ 的旋转矩阵，用于描述坐标系之间的相对姿态；$p \in \mathbb{R}^3$ 是一个 $3 \times 1$ 的平移向量，用于描述坐标系原点之间的相对位置。通过齐次变换矩阵，可以方便地实现坐标点在不同坐标系下的表示转换以及多级坐标变换的复合运算。

\subsubsection{Denavit-Hartenberg 参数法}
...
UR5 机械臂的腕部三轴（关节4、5、6）交于一点，满足 Pieper 准则，因此其逆运动学存在封闭的解析解。求解过程采用解析法，通过解耦思想将位置和姿态问题分开处理。首先，根据末端位姿求出腕部中心点的位置，进而解出前三个关节 $\theta_1, \theta_2, \theta_3$；然后，根据求出的前三个关节计算出腕部相对于基座的姿态，结合期望的末端姿态，解出后三个关节 $\theta_4, \theta_5, \theta_6$。具体求解过程如下：
...
关节角 $\theta_5$ 可通过将 $T_6^0$ 左乘 $(T_1^0)^{-1}$ 得到 $T_6^1$，其位置向量的 $z$ 分量仅与 $\theta_5$ 有关，从而求得，同样存在两组解：

\begin{equation}
  \theta_5 = \pm \text{acos}\left(\frac{p_x s_1 - p_y c_1 - d_4}{d_6}\right)
\end{equation}

关节角 $\theta_6$ 可由 $T_6^1$ 的旋转矩阵元素得到。当 $\sin\theta_5 \neq 0$ 时：

\begin{equation}
  \theta_6 = \text{atan2}\left(\frac{-o_x s_1 + o_y c_1}{\sin\theta_5}, \frac{n_x s_1 - n_y c_1}{\sin\theta_5}\right)
\end{equation}

求出 $\theta_1, \theta_5, \theta_6$ 后，可计算出 $T_3^0 = T_1^0 A_2 A_3$。通过将 $T_6^0$ 左乘 $(T_3^0)^{-1}$ 并右乘 $(T_6^4)^{-1}(T_5^4)^{-1}$，可以得到 $T_4^3$。关节角 $\theta_3$ 可利用余弦定理求得，存在两组解：

\begin{equation}
  \cos(\theta_3) = \frac{\|p_{c}\|^2 - d_1^2 - d_4^2 - a_2^2 - a_3^2}{2 a_2 a_3}
\end{equation}

关节角 $\theta_2$ 可由 $\theta_1$ 和 $\theta_3$ 共同确定：

\begin{equation}
  \theta_2 = \text{atan2}(-p_{cz}, \pm\sqrt{p_{cx}^2+p_{cy}^2-d_4^2}) - \text{atan2}(a_3 s_3, a_2 + a_3 c_3)
\end{equation}

最后，关节角 $\theta_4$ 可由旋转矩阵 $R_6^3 = (R_3^0)^{-1} R_6^0$ 的元素得到：

\begin{equation}
  \theta_4 = \text{atan2}(R_{6,23}^3, R_{6,13}^3)
\end{equation}

由于三角函数的周期性和对称性，UR5 的逆运动学通常存在八组不同的关节角配置，均能使末端达到同一位姿。这种能够快速精确计算所有可能解的能力，对于需要实时轨迹规划的遥操作与实物控制至关重要。
