...
为了有效表征同一观测状态下可能存在多种成功动作轨迹的多模态分布，ACT 引入了一个低维连续隐变量 $z$ 来显式编码演示数据中的风格特征。在给定当前观测序列 $o$ 的条件下，模型的核心目标是最大化未来 $k$ 步专家动作块 $a_{t:t+k-1}$ 的条件对数似然。由于直接积分难以计算，ACT 通过引入近似后验分布 $q_\phi(z|a, o)$ 来构建变分下界（ELBO）。假设隐变量的先验分布服从标准正态分布 $\mathcal{N}(0, I)$，其优化目标可转化为最小化包含重构损失与 KL 散度正则化项的联合损失函数：
...
在闭环动作序列预测上，扩散策略将动作序列预测能力与滚动时域控制（RHC）相结合。在时间步 $t$，以最近 $T_o$ 步的观察数据为输入，一次性预测 $T_p$ 步的动作序列，但仅将前 $T_a$ 步的动作直接执行在机械臂上。这种“看得很远，走得很稳”的设计，平衡了长期轨迹规划与对外界扰动的实时闭环响应。
