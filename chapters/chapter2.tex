...
\begin{equation}
  \theta_1 = \text{atan2}(p_{cy}, p_{cx}) - \text{atan2}(\pm\sqrt{p_{cx}^2 + p_{cy}^2 - d_4^2}, d_4)
\end{equation}
...
\begin{equation}
  \theta_6 = \text{atan2}\left(\frac{-o_x s_1 + o_y c_1}{n_x s_1 - n_y c_1}, \frac{1}{\sin\theta_5}\right)
\end{equation}

求出 $\theta_1, \theta_5, \theta_6$ 后，可计算出 $T_4^3 = (T_3^2)^{-1}(T_2^1)^{-1}(T_1^0)^{-1}T_6^0(T_6^5)^{-1}(T_5^4)^{-1}$，进而求解剩余关节。关节角 $\theta_3$ 可利用余弦定理求得，存在两组解：

\begin{equation}
  \cos(\theta_3) = \frac{K - a_2^2 - a_3^2}{2 a_2 a_3}
\end{equation}

其中 $K$ 是一个由已知量构成的中间变量。关节角 $\theta_2$ 和 $\theta_4$ 则可由其他已知关节角共同确定。由于三角函数的周期性和对称性，UR5 的逆运动学通常存在八组不同的关节角配置，均能使末端达到同一位姿。这种能够快速精确计算所有可能解的能力，对于需要实时轨迹规划的遥操作与实物控制至关重要。
