...
模仿学习旨在通过学习人类示范数据，使机械臂能够快速掌握复杂任务技能，从而有效规避强化学习中常见的大量试错过程。其核心在于利用专家提供的状态与动作序列作为监督信号，训练智能体的策略模型，使其在面临相似状态时能够输出与专家一致的动作。

在理论框架中，系统通常被建模为马尔可夫决策过程。给定状态空间 $\mathcal{S}$ 和动作空间 $\mathcal{A}$，专家的策略可表示为条件概率分布 $\pi_E(a|s)$。训练时，算法接收由专家提供的演示轨迹集合 $\mathcal{D} = \{\tau_1, \tau_2, \dots, \tau_N\}$，其中每条轨迹 $\tau_i = \{(s_1, a_1), (s_2, a_2), \dots, (s_T, a_T)\}$ 包含了从初始状态到终止状态的映射序列。

传统行为克隆是模仿学习中最基础的方式。它将策略学习转化为监督学习问题，通过分类器或回归器复制专家策略，寻找参数化策略 $\pi_\theta(a|s)$ 使得其输出动作与专家动作之间的误差最小化。以确定性策略为例，其优化目标可表示为：

\begin{equation}
  \theta^* = \arg\min_\theta \mathbb{E}_{(s,a) \sim \mathcal{D}} \left[ \mathcal{L}(a, \pi_\theta(s)) \right]
\end{equation}

其中，$\mathcal{L}$ 为损失函数，例如均方误差。尽管行为克隆方法简单，但其存在明显的局限性。首先，该方法依赖于示范者的动作序列，而在现实场景中，往往只能观察到人类模仿的动作效果，难以获取其内部控制信号，限制了其应用范围。其次，行为克隆易受协变量转移影响。由于模型预测的微小误差会随时间累积，导致机械臂实际遇到的状态分布逐渐偏离训练时的示范数据分布，进而造成性能降低。此外，传统的行为克隆通常只能学习单一行为策略，难以适应复杂多变的环境与多样化任务，降低了机械臂执行任务的灵活性与成功率。

为了克服这些局限性，基于 Transformer 架构和扩散模型的模仿学习方法逐渐兴起，并在解决精细操作与复杂动作分布问题上展现出显著优势。
